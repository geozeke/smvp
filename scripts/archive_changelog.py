...
def parse_version(version: str) -> tuple[int, int, int]:
    ...
    ----------
    version
        Semantic version with or without a leading ``v``.
...
def format_changelog(
    ...
    ----------
    preamble
        Text that appears before the first release section.
    releases
        Release sections to include.
    link_definitions
        Link reference definitions available to the rendered content.

    Returns
    -------
    str
        Rendered changelog content with a trailing newline.
    """
    parts: list[str] = []
    if preamble:
...
def should_archive(
    ...
    ----------
    target_minor
        Major/minor line that should remain active.
    releases
        Release sections found in newest-first order.
    force
        Whether to archive even when this looks like a patch update.

    Returns
    -------
    bool
        ``True`` when archiving should run.
    """
    if force:
        return True
    if len(releases) < 2:
...
def archive_changelog(
    ...
    ----------
    version
        Target semantic version, with or without a leading ``v``.
    changelog_path
...
def main() -> None:
    """Run the changelog archiver from the command line."""
    parser = argparse.ArgumentParser(
        description="Archive CHANGELOG.md entries outside a semantic minor line."
    )
    parser.add_argument("version", help="Target semantic version, e.g. v0.5.0.")
    parser.add_argument(
        "--changelog",
        type=Path,
        default=Path("CHANGELOG.md"),
        help="Path to the active changelog.",
    )
    parser.add_argument(
        "--archive-dir",
        type=Path,
        default=Path("changelogs"),
        help="Directory for archived minor-version changelogs.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Archive non-target minor lines even during a patch update.",
    )
    args = parser.parse_args()
    archive_changelog(args.version, args.changelog, args.archive_dir, args.force)
